...
#            return -1 # -1 corrisponde a cella non valida
    ...
    if wordToCheck in italianDictionary:
        if not (wordToCheck in validWords):
            ...
            if 0:
                #PARSING STAGE
                parsedCellsCoordinates=""
                for couple in cellsToFormWord:
                    parsedCellsCoordinates+=str(couple[0])+str(couple[1])+"-"
                validCoordinates.append(parsedCellsCoordinates[:-1]) #Tolgo l'ultima '-' in fondo alla stringa
                #END OFPARSING STAGE
...
def main():
    ...
    #Inverto (validCoordinates lo inverto direttamente nel return)
    pdb.set_trace()
    validWords=sorted(validWords,key=len,reverse=True)

    #ANTIPARSING STAGE
    if 0:entriesToDisplay=[nonParsedWord.split('-') for nonParsedWord in validCoordinates]
    #END OF ANTIPARSING STAGE
...
def solveGame():
    ...
    if 0:
        coordinatesOfEntriesToColour=[]
        for index,listOfCoordinates in enumerate(entriesToColour):
            coordinatesOfEntriesToColour.append([])
            for entry in listOfCoordinates:
                coordinatesOfEntriesToColour[index].append(entryList[int(entry[0])][int(entry[1])])
        allEntries=itertools.chain(coordinatesOfEntriesToColour)
...
def changebg(listOfAllEntries):
    ...
    if entry is not None:
        entry['bg']='yellow'
        root.after(500,lambda : changebg(listOfAllEntries))
