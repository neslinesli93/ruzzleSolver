...
def changebg(listOfAllEntries,initialDelay,delay,colour):
    ...
    if entry is not None:
        entry['bg']=colour
        root.after(initialDelay,changebg,listOfAllEntries,delay,delay,colour)
        return 1

def divideListToColour(setOfAllEntries):
    listToSplit=next(setOfAllEntries,None)
    if listToSplit is not None:
        result=changebg(listToSplit,500,500,'yellow')
        
        changebg(listToSplit,len(listToSplit)*500,0,'white')
